...
const ModalUpdate = ({ title, open, onCancel, updateId }) => {
  ...
  const editHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(api.seller.update + updateId, dataUpload, {
  ...
  useEffect(() => {
    if (updateId !== undefined && updateId !== null && updateId !== "") {
      axios
        .get(api.seller.getById + updateId)
        .then((data) => {
          let tempData = [];
          tempData.push(data.data.data);
          setSellerName(tempData[0].seller_name);
          setSellerAddress(tempData[0].seller_address);
          setSellerPhone(tempData[0].seller_phone);
          setSellerDescription(tempData[0].seller_description);
        })
        .catch((err) => {
